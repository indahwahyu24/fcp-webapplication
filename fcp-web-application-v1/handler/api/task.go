...
func (t *taskAPI) UpdateTask(c *gin.Context) {
	...
	var makeTaskNew model.Task
	if err := c.ShouldBindJSON(&makeTaskNew); err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: err.Error()})
		return
	}

	var id int
	makeTaskNew.ID = taskID
	err = t.taskService.Update(id, &makeTaskNew)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
...
func (t *taskAPI) GetTaskListByCategory(c *gin.Context) {
	...
	tasks, err := t.taskService.GetTaskCategory(category)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
